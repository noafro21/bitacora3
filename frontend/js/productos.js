...
async function cargarDatosUsuario() {
  try {
    ...
    mostrarError(
      "No se pudieron cargar los datos del usuario. Por favor, intenta nuevamente.",
    );
    return null;
  }
}
...
function renderizarProductos() {
  ...
  productos.forEach((producto, index) => {
    const colorProducto = determinarColorProducto(
      producto.puntosNecesarios || 0,
    );
    const textoEstado = obtenerTextoEstado(colorProducto);
    const puedeCanjear = colorProducto === "verde";
    const puntosFaltantes = Math.max(
      0,
      (producto.puntosNecesarios || 0) - puntosDisponibles,
    );
    const porcentajeCompletado =
      (puntosDisponibles / (producto.puntosNecesarios || 1)) * 100;
...
async function actualizarDatos() {
  // Cargar datos
  const [_resultadoUsuario, resultadoProductos] = await Promise.all([cargarDatosUsuario(), cargarProductos()]);

  // Renderizar productos solo si la carga fue exitosa
